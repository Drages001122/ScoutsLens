...
def get_player_stats(game_id):
    ...
    try:
        ...
        if len(teams_in_game) == 2:
            ...
            home_team = player_stats[player_stats["teamTricode"] == team1][
                "teamName"
            ].iloc[0]
            away_team = player_stats[player_stats["teamTricode"] == team2][
                "teamName"
            ].iloc[0]
            home_score = score1
            away_score = score2
        ...
        # 计算两分数据（总投篮 - 三分投篮）
        player_stats["twoPointersMade"] = (
            player_stats["fieldGoalsMade"] - player_stats["threePointersMade"]
        )
        player_stats["twoPointersAttempted"] = (
            player_stats["fieldGoalsAttempted"] - player_stats["threePointersAttempted"]
        )
...
def main():
    ...
    # 去重操作 - 基于球员ID和比赛相关字段去除重复记录
    # 检查是否包含GAME_ID字段
    if "GAME_ID" in combined_stats.columns:
        # 使用球员ID和比赛ID的组合去重
        duplicate_count = len(combined_stats) - len(
            combined_stats.drop_duplicates(subset=["personId", "GAME_ID"])
        )
        combined_stats = combined_stats.drop_duplicates(
            subset=["personId", "GAME_ID"], keep="first"
        )
    else:
        # 如果没有GAME_ID，使用球员ID、比赛日期和球队代码的组合去重
        duplicate_count = len(combined_stats) - len(
            combined_stats.drop_duplicates(
                subset=["personId", "GAME_DATE", "teamTricode"]
            )
        )
        combined_stats = combined_stats.drop_duplicates(
            subset=["personId", "GAME_DATE", "teamTricode"], keep="first"
        )

    if duplicate_count > 0:
        print(f"去除了 {duplicate_count} 条重复记录")
    ...
    # 选择字段（不重命名）
    formatted_stats = combined_stats[required_fields]
