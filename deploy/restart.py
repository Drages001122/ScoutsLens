...
def backup_database_before_restart(
    ...
    backup_file = f"{backup_dir}/scoutslens_restart_backup_{timestamp}.db"
...
def restart_service():
    ...
    print("=" * 60)
    print("重启 ScoutsLens FastAPI 服务")
    print("=" * 60)
    print(f"服务器: {hostname}")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    backup_success = backup_database_before_restart(
        hostname, username, password, port, remote_project_dir
    )
    if not backup_success:
        print("⚠ 数据库备份失败，但继续重启服务...")

    commands = [
        ("重启 FastAPI 应用服务", "systemctl restart scoutslens"),
